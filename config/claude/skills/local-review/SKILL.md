...
If told to run in `fast` mode (eg. `/local-review fast`) then do not run tests or any other expensive operations.
Remind me of this in the summary.

### Project Specific Rules

#### tms-ux

- Ensure error handling uses `extractMutationErrors`
- Ensure modal handling uses `useSimpleModal`
- Ensure graphql queries use `graphql()` not `gql`
...
Finally rate how safe these changes are to be deployed to production out of 10.
10 is the highest confidence score, the changes are purely additive or documentation updates.
0 is the lowest confidence score, the changes are definitely breaking or highly risky.
